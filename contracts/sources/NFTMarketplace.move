address 0x223d508f051f5869e232658de4a25c493813273319b5130ae54838c609be630d {
    ...
    module NFTMarketplace {
        ...
        // New feature: Support or donation for creators who have minted at least one NFT
        public entry fun tip_creator(account: &signer, creator: address, amount: u64) acquires Marketplace {
            let marketplace = borrow_global<Marketplace>(@0x223d508f051f5869e232658de4a25c493813273319b5130ae54838c609be630d);
            let creator_has_nft = false;
            ...
            while (mut_i < nfts_len) {
                ...
                if (nft.owner == creator) {
                    creator_has_nft = true;
                    break
                };
                mut_i = mut_i + 1;
            };

            assert!(creator_has_nft, 500); // No NFTs found for this creator

            // Transfer tip to creator
            coin::transfer<aptos_coin::AptosCoin>(account, creator, amount);
        }

        // New Feature: NFT transfer
        public entry fun transfer_nft(account: &signer, marketplace_addr: address, nft_id: u64, new_owner: address) acquires Marketplace {
            ...
            assert!(nft_id < nfts_length, 1002); // NFT does not exist
    
            let nft = vector::borrow_mut(&mut marketplace.nfts, nft_id);
    
            // Check if the caller is the owner
            assert!(signer::address_of(account) == nft.owner, 1000); // Only the owner can transfer
    
            // Prevent transfer to self
            assert!(new_owner != nft.owner, 1001); // Cannot transfer to self
    
            // Prevent transfer to zero address
            assert!(new_owner != @0x0, 1003); // Cannot transfer to the zero address
    
            nft.owner = new_owner;
            nft.for_sale = false; // Reset for_sale status
